				...
				(function (global) {
					...
					var contract = (function (module) {
						...
						Provider.prototype.send = function () {
							return this.provider.send.apply(
								this.provider,
								arguments
							);
						};
						...
						Provider.prototype.sendAsync = function () {
							return this.provider.sendAsync.apply(
								this.provider,
								arguments
							);
						};
						...
						var Utils = {
								...
								return logs
									.map(function (log) {
										...
										function partialABI(fullABI, indexed) {
											var inputs = fullABI.inputs.filter(
												function (i) {
													return (
														i.indexed === indexed
													);
										...
										copy.args = logABI.inputs.reduce(
											function (acc, current) {
												var val =
													indexedParams[current.name];
							...
							bootstrap: function (fn) {
								...
								Object.keys(fn._static_methods).forEach(
									function (key) {
										fn[key] =
											fn._static_methods[key].bind(fn);
									}
								);

								// Add our properties.
								Object.keys(fn._properties).forEach(function (
									key
								) {
						...
						function Contract(contract) {
							...
							if (typeof contract == "string") {
								...
								var contract_class =
									constructor.web3.eth.contract(this.abi);
							...
							for (var i = 0; i < this.abi.length; i++) {
								...
								if (item.type == "function") {
									if (item.constant == true) {
										this[item.name] =
											Utils.promisifyFunction(
												contract[item.name],
												constructor
											);
									} else {
										this[item.name] =
											Utils.synchronizeFunction(
												contract[item.name],
												this,
												constructor
											);
									...
									this[item.name].request =
										contract[item.name].request;
						...
						Contract._static_methods = {
							...
							new: function () {
								...
								return self
									...
									.then(function (network_id) {
										...
										var regex = /__[^_]+_+/g;
										var unlinked_libraries =
											self.binary.match(regex);
										...
										return new Promise(function (
											...
											var intermediary = function (
												...
												if (
													err == null &&
													web3_instance != null &&
													web3_instance.address !=
														null
												) {
													accept(
														new self(web3_instance)
													);
												}
											};

											args.push(tx_params, intermediary);
											contract_class.new.apply(
												contract_class,
												args
											);
										});
							...
							defaults: function (class_defaults) {
								...
								var self = this;
								Object.keys(class_defaults).forEach(function (
									key
								) {
							...
							hasNetwork: function (network_id) {
								return (
									this._json.networks[network_id + ""] != null
								);
							},
							...
							isDeployed: function () {
								...
								if (
									this._json.networks[this.network_id] == null
								) {
									return false;
								}
							...
							detectNetwork: function () {
								...
								return new Promise(function (accept, reject) {
									...
									self.web3.version.getNetwork(function (
										err,
										result
									) {
							...
							link: function (name, address) {
								...
								if (typeof name == "function") {
									...
									Object.keys(contract.events).forEach(
										function (topic) {
											self.network.events[topic] =
						...
						Contract._properties = {
							...
							updated_at: function () {
								try {
									return (
										this.network.updated_at ||
										this._json.updated_at
									);
								} catch (e) {
		...
		2: [
			function (require, module, exports) {
				...
				var contract = function (options) {
					...
					var binary = Schema.generateBinary(
						options,
						{},
						{ dirty: false }
					);
				...
				contract.fromSolJS = function (
					...
					Object.keys(soljs_abstraction.all_networks).forEach(
						function (network_name) {
							...
							if (
								soljs_abstraction.all_networks[network_name]
									.updated_at > latest_network_updated_at
							) {
							...
							[
								...
								networks[network_name][key] =
									soljs_abstraction.all_networks[
										network_name
									][key];
							});
						}
					...
					latest_network =
						soljs_abstraction.all_networks[latest_network] || {};
				...
				(function (Buffer) {
					...
					function decodeParams(names, types, data) {
						...
						types.forEach(function (type, index) {
							var coder = getParamCoder(type);
							if (coder.dynamic) {
								var dynamicOffset = uint256Coder.decode(
									data,
									offset
								);
								var result = coder.decode(
									data,
									dynamicOffset.value.toNumber()
								);
					...
					function decodeEvent(eventObject, data) {
						var inputNames = utils.getKeys(
							eventObject.inputs,
							"name",
							true
						);
						var inputTypes = utils.getKeys(
							eventObject.inputs,
							"type"
						);
			...
			function (require, module, exports) {
				(function (Buffer) {
					...
					function hexlify(value) {
						if (typeof value === "number") {
							return (
								"0x" + bnToBuffer(new BN(value)).toString("hex")
							);
					...
					var coderString = {
						encode: function encodeString(value) {
							return encodeDynamicBytesHelper(
					...
					function coderArray(coder, lengthInput) {
						return {
							...
							decode: function decodeArray(data, offsetInput) {
								...
								if (length === -1) {
									decodeResult = uint256Coder.decode(
										data,
										offset
									);
				...
				(function (process, global) {
					...
					(function (root) {
						...
						var RC = [
							...
							2147483658, 2147483648, 2147516545, 2147483648,
						...
						var createShakeMethod = function (bits, padding) {
							...
							for (var i = 0; i < OUTPUT_TYPES.length; ++i) {
								var type = OUTPUT_TYPES[i];
								method[type] = createShakeOutputMethod(
									bits,
									padding,
									type
								);
						...
						Keccak.prototype.update = function (message) {
							...
							if (
								notString &&
								message.constructor == root.ArrayBuffer
							) {
			...
			function (require, module, exports) {
				...
				var TruffleSchema = {
					...
					normalizeOptions: function (options, extra_options) {
						...
						if (
							normalized.unlinked_binary == null &&
							normalized.binary
						) {
					...
					generateBinary: function (
						...
						existing_binary.abi =
							options.abi || existing_binary.abi;
						existing_binary.unlinked_binary =
							options.unlinked_binary ||
							existing_binary.unlinked_binary;
						...
						existing_binary.networks =
							existing_binary.networks || {};
						options.networks = options.networks || {};
						Object.keys(options.networks).forEach(function (
							network_id
						) {
						...
						if (options.network_id) {
							...
							existing_binary.abi.forEach(function (item) {
								...
								network.events[
									"0x" +
										sha3(signature, { outputLength: 256 })
								] = item;
							});
							...
							if (options.address) {
								throw new Error(
									"Cannot set address without network id"
								);
							}
						}
		...
		20: [
			function (require, module, exports) {
				module.exports = {
					...
					homepage:
						"https://github.com/trufflesuite/truffle-schema#readme",
					keywords: [
						"ethereum",
						"json",
						"schema",
